<section class="flex items-center justify-center pb-32 pt-10">
    <div class="max-w-[900px]">
        ...
        <p>The process of image reconstruction is widely used within the medical fields for reconstructing data from a CT scan to show what the inside of a person looks like. This ability to see inside a person allows for doctors to identify problems within the body and treat them appropriately. Another field image re construction is also used in is nondestructive imaging of parts such as on airplanes. This type of imaging allows for checking on the internals of certain structural areas of the plane that would otherwise be expensive as well as damage structural integrity if done. Within this report the basic algorithm for filtered back projecting and comparing this to non-filtered back projections. This type of filter allows for a clearer image after back projecting, and it is the reason why it is filtered. To compare filtered and non-filtered reconstructions a standard reference for computed tomography will be used, the shepp-logan phantom [10].</p>
        <h1 id="applications">Applications</h1>
        <p>The main application of image reconstruction is when an internal view of an object or person is needed but it is either too costly or dangerous to do so. This type of nondestructive and noninvasive view of is useful in several fields. One of the main fields for this is medical imaging. The main use for this type of imaging, is in C.T scanners and MRI machines [11]. These machines allow for noninvasive views in to the body rather than the more invasive biopsy[11].
        Another field where tomography is used is within ocean observation, measuring temperatures and currents. This form of tomography use acoustic waves instead of the higher power x-rays as used in medicine [12]. This form of tomography uses the changing speed of sound within the ocean to not only determine ocean temperatures but also depth.</p>
        <h1 id="background">Background</h1>
        <p>Image reconstruct works because of the “Fourier projection theorem” or Projection-slice theorem” [13]. This theorem states that the integral of a shape projection along some axis is a slice of the 2d Fourier transform of that shape projected along the same axis. A graphical view of this theorem can be seen in Figure 1. This method is called back projection. Plotting all these back projected angles creates a sinogram and example of which can be seen in Figure 2. This figure shows sinogram of a basic test image that can be found in Appendix B: basic image This theorem also states that an infinite number of projects are taken at different angles and the image can be perfectly reconstructed. </p>
        
        <div class="flex items-center justify-center">
            <img src="/tomo/tomo-image-1.png" alt="alt text">
        </div>
        
        <p>
        Figure 1: example of Fourier projection theorem source: Public Domain, <a href="https://commons.wikimedia.org/w/index.php?curid=646788">https://commons.wikimedia.org/w/index.php?curid=646788</a></p>
        <p>However, as an infinite number of projections a very large number of angles must be computed to reconstruct the image. Another problem occurs with this projection, as the rotation happens around the origin in Fourier space points near the center in Fourier space have more samples where points further away from the center tend to be more sparsely populated. This is due to the polar sampling in Fourier space. Therefor interpolating between these values is needed to create a clearer image. This is done through a filtering step that uses a ramp filter in the Fourier domain to interpolate between these values.</p>
        
        <div class="flex items-center justify-center">
            <img src="/tomo/tomo-image-2.png" alt="alt text">
        </div>
        
        <p>
        Figure 2: sinogram of basic image</p>
        <p>As can be seen in this Figure 2: sinogram of basic image the sinogram is mirror at the 180° mark, however it is mirrored both horizontally and vertically. using this knowledge this means that when taking the back projections of an object, only must take the back projections of the object from 0 to 180 degrees to have enough data to reconstruct the object. To take these back projected and reconstructed the image from them a method must be used called forward projection. This technique takes the slice and angle and project the values at that angle across the image plane, as can be seen in Figure 3.
        </p> 
        <div class="flex items-center justify-center">
            <img src="/tomo/tomo-image-3.png" alt="alt text">
        </div>    
        <p>
        Figure 3: forward projection of basic image, with 6 projections</p>
        <h1 id="back-projection">Back projection</h1>
        <p>As previously discussed with in the Background, to reconstruct and image firstly the image must be back projected to create a sinogram. Although this method will lose value due to polar sampling within the Fourier slice theorem. Using the reference image that was outlined within the Introduction, the sinogram of this image can be seen in Figure 4. The reconstruction from this sinogram can be seen in Figure 5.
        
        </p>    
        <div class="flex items-center justify-center">
            <img src="/tomo/tomo-image-4.png" alt="alt text">
        </div>    
        <p>
        Figure 4: sinogram of shepp-logan phantom
        </p>
        <div class="flex items-center justify-center">
            <img src="/tomo/tomo-image-5.png" alt="alt text">
        </div>
        <p>
        Figure 5: reconstruction of shepp-logan phantom </p><p>
        This reconstruction is very blur and hard to make out the original image. The reconstruction also has a circle around it most likely from the forward projection stage where the image was rotated and stacked. However, the original shepp-logan phantom can still be made out although some of the smaller details were lost.</p>
        <h1 id="filtered-back-projection-">Filtered back projection.</h1>
        <p>The 2d Fourier transform of an image is defined in equation 30:
        <!-- $$
        f\left(x,y\right)=\int<em>{-\infty}^{\infty}\int</em>{-\infty}^{\infty}F\left(u,v\right){e}^{{j}\mathbf{2\pi}\left({ux}+{vy}\right)}{dudv}\ \ \ \ (1)
        $$
        This equation can then be rewritten using the projection theorem as seen in equation 1:
        $$
        {f}\left({x},{y}\right)=\int<em>{0}^{2\pi}\int</em>{0}^{\infty}F\left(\delta cos\left(\theta\right),\delta s i n\left(\theta\right)\right){e}^{{j}\mathbf{2\pi}\left(xcos\left(\theta\right)+sin\left(\theta\right){y}\right)}\delta{d}\delta{d}\theta\ \ (2)
        $$
        ...
        $$
        {f}\left({x},{y}\right)=\int<em>{0}^{2\pi}\int</em>{0}^{\infty}p\left(\theta,\delta\right){\left|\delta\right|{e}}^{{j}\mathbf{2\pi}\delta r}{d}\delta{d}\theta\ \ \ (3)
        $$ -->
        ...
        <div class="flex items-center justify-center">
            <img src="/tomo/tomo-image-6.png" alt="alt text">
        </div>
        <p>
        ...
        <div class="flex items-center justify-center">
            <img src="/tomo/tomo-image-7.png" alt="alt text">
        </div>
        <p>
        ...
        There are also circles that appear around both filtered and non-filtered reconstruction. These circles are more than likely to come from the forward projection stage when the image matrix is rotated. This would occur as during this stage the forward projected image is resampled as well as receiving some interpolation.</p>
